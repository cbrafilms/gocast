...
try {
  ...
  $pdo->exec("CREATE TABLE IF NOT EXISTS shortlists (
    ...
    cliente_access_password VARCHAR(64) NULL,
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // evitar shortlist duplicado con mismos talentos para el mismo casting
    $existing = $pdo->prepare('SELECT id, talentos_json FROM shortlists WHERE casting_id = ? AND productora_id = ?');
    $existing->execute([$castingId, $productoraId]);
    $newIds = array_map(fn($t) => (string)($t['talento_id'] ?? ''), $talentos);
    sort($newIds);
    foreach (($existing->fetchAll() ?: []) as $row) {
      $old = json_decode($row['talentos_json'] ?? '[]', true) ?: [];
      $oldIds = array_map(fn($t) => (string)($t['talento_id'] ?? ''), $old);
      sort($oldIds);
      if ($oldIds === $newIds) {
        json_response(400, ['detail' => 'Ya existe un shortlist con esos mismos talentos para este casting']);
      }
    }

    $urlPublica = bin2hex(random_bytes(8));
    $accessPassword = strtoupper(substr(bin2hex(random_bytes(4)), 0, 8));
    $ins = $pdo->prepare('INSERT INTO shortlists (casting_id, productora_id, nombre, url_publica, talentos_json, cliente_access_password) VALUES (?, ?, ?, ?, ?, ?)');
    $ins->execute([$castingId, $productoraId, $nombre, $urlPublica, json_encode($talentos, JSON_UNESCAPED_UNICODE), $accessPassword]);
    ...
    json_response(200, [
      ...
      'cliente_access_password' => $accessPassword,
      'talentos' => $talentos,
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $q = $pdo->prepare('SELECT id, nombre, url_publica, talentos_json, cliente_seleccion_json, cliente_finalizado, cliente_access_password, fecha_creacion FROM shortlists WHERE casting_id = ? AND productora_id = ? ORDER BY id DESC');
    $q->execute([$castingId, $productoraId]);
    $rows = $q->fetchAll() ?: [];
    ...
    $out = array_map(function($r){
      return [
        ...
        'cliente_access_password' => $r['cliente_access_password'],
